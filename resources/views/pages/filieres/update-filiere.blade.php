...
<div class="w-full max-w-3xl bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 md:p-8 relative">
    <h2 class="text-2xl font-bold mb-6 text-center text-blue-500">Modifier une  Filières</h2>
 <a href="{{ route('filieres.index') }}" class="absolute right-0 top-0 bg-red-400 text-white px-3 py-2 rounded hover:text-red-200">Back</a>
    <form action="{{ route('filieres.update', $filiere->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" id="nom" name="name" value="{{ old('name', $filiere->name) }}" placeholder="Ex : Informatique" required
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="4" placeholder="Décrivez la filière ici..." required
                class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white resize-none">{{ old('description', $filiere->description) }}</textarea>
                 @error('description')
                <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                @enderror
        </div>
