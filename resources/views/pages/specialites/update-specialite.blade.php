...
<section
    ...
    <div class="w-full relative max-w-2xl bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 md:p-8">
        ...
        <form action="{{ route('specialites.update', $specialite->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="">
                ...
                <select name="filiere_id" required
                    ...
                    <option value="{{ $filiere->id }}" {{ $specialite->filiere_id == $filiere->id ? 'selected' : '' }}>
                        {{ $filiere->name }}
                    </option>
                    @endforeach
                </select>
